...
### Tone References
- Notion (warm paper whites, editorial structure, unhurried reading)
- Stripe Docs (cream-white surfaces, ink-dark type, purposeful green accents)
- Basecamp (calm, trusted, warm off-white product feel)
- Monocle magazine (print editorial warmth, typography-first)
- Ramp (premium fintech lightness — cream backgrounds, deep green trust signals)

---

## 2. Design Tokens

> **Color story:** Warm parchment — the palette recalls a well-worn research notebook: cream paper, ink-dark text, and a deep forest green that feels authoritative without being cold. Backgrounds are warm off-whites that never feel clinical. The accent (`#1B5E3B` → `#2D7A52`) carries trust and precision — like a researcher's pen on cream paper. Teal (`#0D9488`) signals the system thinking: pipeline pulses, AI activity. Nothing screams. Everything earns attention through contrast and weight, not colour loudness.

### 2.1 Color Palette

```
Theme: Light-first. Warm crema — parchment, ink, forest.
All surfaces are warm off-whites. Never pure white, never clinical grey.

Background layers (z-axis stacking):
  --bg-base:        #FAF8F3   // page background — warm cream parchment
  --bg-surface:     #F4F0E8   // cards, panels — slightly deeper cream
  --bg-elevated:    #EDE8DC   // modals, input fills, active row bg
  --bg-subtle:      #F7F4EE   // sidebar, nav — lightest warm tone

Borders:
  --border-dim:     #E5DFD3   // hairline dividers — barely there
  --border-default: #D4CCBF   // standard card/input borders
  --border-active:  #B8AE9C   // focused inputs, hovered cards

Text:
  --text-primary:   #1A1510   // rich warm black — ink on paper
  --text-secondary: #5C5448   // warm mid-brown — body copy
  --text-muted:     #9C9080   // warm light — timestamps, metadata
  --text-inverse:   #FAF8F3   // on dark/accent backgrounds

Accent — Deep Forest Green (the hero color):
  --accent-500:     #1B5E3B   // deep forest: primary CTA, focus rings, active states
  --accent-400:     #2D7A52   // hover states on accent elements
  --accent-300:     #4CAF75   // lighter green for highlights, show-more links
  --accent-100:     #E8F4EC   // very soft green fill — tag/badge backgrounds
  --accent-900:     #0D3320   // darkest forest — text on light green fills

Secondary — Teal (pipeline, AI activity):
  --teal-600:       #0D9488   // pipeline running pulse, processing states
  --teal-200:       #99F6E4   // teal highlights
  --teal-100:       #CCFBF1   // teal badge backgrounds

Semantic:
  --accepted:       #1B5E3B   // accepted paper — deep forest green
  --accepted-bg:    #E8F4EC
  --processing:     #0D9488   // pipeline running/active — teal
  --processing-bg:  #CCFBF1
  --dismissed:      #6B7280   // rejected/dismissed — neutral slate
  --dismissed-bg:   #F1F2F4
  --info:           #1D4ED8   // blue for read-only system info

Score colors (agent_score 0–10):
  --score-high:     #1B5E3B   // 8.0–10.0  deep forest green
  --score-mid:      #0D9488   // 5.0–7.9   teal
  --score-low:      #9C9080   // 0.0–4.9   warm muted (not grey, not yellow)
```
...
```
--shadow-sm:    0 1px 2px rgba(26,21,16,0.06), 0 1px 4px rgba(26,21,16,0.04)
--shadow-md:    0 4px 12px rgba(26,21,16,0.08), 0 2px 6px rgba(26,21,16,0.05)
--shadow-lg:    0 16px 40px rgba(26,21,16,0.12), 0 4px 12px rgba(26,21,16,0.06)
--glow-accent:  0 0 0 3px rgba(27,94,59,0.12)    // focus ring on cards and inputs
--glow-strong:  0 0 20px rgba(27,94,59,0.10)     // hero elements, CTA hover
```
...
- Running: animated pulsing teal dot + "Running · 65%" in `--teal-600`, subtle `--teal-100` bg fill on pill
...
- Subtle warm radial gradient: `rgba(27,94,59,0.04)` at top-center → transparent — reads as natural light through a window
- Fine dot grid: `radial-gradient(circle, rgba(27,94,59,0.08) 1px, transparent 1px)` at 32px spacing
- Together: parchment warmth with a breath of nature — like a quality paper stock
...
- Category tags: `--accent-100` bg, `--accent-900` text, `--radius-sm`, `--text-xs` monospace uppercase
...
- Agent explanation block: `--accent-100` bg, left `3px` border `--accent-500`, `--radius-sm`, py=12 px=16, `--text-sm` `--text-secondary`
...
- `#fff` background, `--border-default` border, `--radius-xl`, `--shadow-lg`
- Backdrop: `rgba(26,21,16,0.4)` blur(6px) — warm ink veil over parchment
...
- "Explain" button: `--accent-500` border, `--accent-500` text, `--accent-100` bg, icon: sparkles ✦
- If explanation exists: `--accepted` badge "Explained" in top-right corner
...
- Loading state: 3-line skeleton shimmer using `--bg-elevated` → `--bg-surface` → `--bg-elevated` gradient
...
- Selected: `--border-active` border, `--accent-100` bg tint, `--accent-500` bullet
...
- Card: `#fff` background, `--border-default` border, `--radius-xl`, `--shadow-md`
...
Green:       bg: --accent-100    |  text: --accent-900                // category tags, key labels
Accepted:    bg: --accepted-bg   |  text: --accepted                  // "Accepted" state chip
Processing:  bg: --processing-bg |  text: --teal-600                  // pipeline state, "AI active"
Dismissed:   bg: --dismissed-bg  |  text: --dismissed                 // rejected papers
Info:        bg: #EFF6FF         |  text: --info                      // read-only system info
...
Outer track:  h=3px, --border-dim, --radius-full, overflow:hidden
Inner fill:   width = (score/10)*100%, --radius-full
              color interpolation via hsl():
              score < 5 → --score-low  (#9C9080 warm muted)
              score < 8 → --score-mid  (#0D9488 teal)
              score ≥ 8 → --score-high (#1B5E3B deep forest green)
...
PENDING:
  dot: 8px, --teal-600, slow pulse (1.5s), opacity 0.6→1
  label: "Queued"

PROCESSING_PDFS / RUNNING / etc:
  dot: 8px, --teal-600, fast pulse (0.8s)
  outer ring: 16px, --teal-600 at 20% opacity, animated scale 1→1.8 + fade (1s loop)
  pill bg: --teal-100 fill
  label: state name monospace lowercase → "processing pdfs"
  progress bar: thin 2px horizontal line below label, --teal-600 fill, animated width
...
Success: --accepted left border (3px) + icon, --accepted-bg bg
Dismiss: --dismissed left border (3px) + icon, --dismissed-bg bg
Info:    --accent-500 left border (3px) + icon, --accent-100 bg
```
